...
test('导演分组自动按顺序命名', () => {
  ...
  assert.deepEqual(state.directorGroups.filter(group => !group.fixed).map(group => group.name), ['分组 1', '分组 2']);
});

test('导演项目可以重命名并移动到分组', () => {
  let state = createDirectorGroup(withProject());
  const groupId = state.directorGroups.find(group => !group.fixed).id;
...
test('分组可以重命名，删除分组只把项目移回未分组', () => {
  ...
  assert.equal(state.directorGroups.filter(group => !group.fixed).length, 0);
  assert.equal(state.directorProjects[0].groupId, 'director-workbench');
});
