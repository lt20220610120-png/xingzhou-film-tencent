...
async function requestChat({endpoint,apiKey='',model,messages,requiresApiKey=true},{fetchFn=fetch,timeout=30000}={}){
 ...
 const response=await fetchFn(chatCompletionsUrl(endpoint),{method:'POST',headers,body:JSON.stringify({model:model.trim(),messages,temperature:.2}),signal:AbortSignal.timeout(timeout)});
