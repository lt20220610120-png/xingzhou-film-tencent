...
const {normalizeEndpoint,chatCompletionsUrl,requestChat,testAiConnection}=require('./ai-service.cjs');
...
test('分析请求可以覆盖普通聊天的30秒超时',async()=>{
 let signal;
 const fetchFn=async(_url,options)=>{signal=options.signal;return {ok:true,text:async()=>JSON.stringify({choices:[{message:{content:'ok'}}]})}};
 await requestChat({endpoint:'http://localhost/v1',model:'demo',messages:[],requiresApiKey:false,timeout:600000},{fetchFn});
...
test('推理过程不冒充正文，兼容 choices.text 与内容数组',async()=>{
 ...
 await assert.rejects(() => requestChat(base,{fetchFn:response({choices:[{message:{content:'',reasoning_content:'推理正文'}}]})}), /只返回了推理过程/);
