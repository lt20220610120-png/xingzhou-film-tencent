...
import {activeMediaProfile, generationMediaProfiles, isFeituoEndpoint, videoModelCapabilities, IMAGE_FORMATS} from '../../core/canvasStore.js';
import {numberedReferences, bindReferencePrompt, mediaSource, appendImportedReferences} from '../../core/generationReferences.js';

export function GenerationComposer({state,api,kind='video',value,onChange,onSubmit,disabled=false,onPick,children}) {
  ...
  const profile=profiles.find(p=>p.id===value.profileId) || activeMediaProfile(state,kind);
  ...
  const caps=feituo ? model : kind==='video' ? videoModelCapabilities(model.id) : {ratios:IMAGE_FORMATS.map(x=>x.value),resolutions:[]};
  ...
  const importFile=async mediaKind=>{
    ...
    }catch(e){setError(e.message);}finally{importLock.current=false;setImporting(false);}
  };
  const run=async()=>{
    ...
    try{
      ...
      await onSubmit({...value,prompt,originalPrompt:value.prompt,profileId:profile.id,kind,model:model.id,endpoint:profile.endpoint,apiKey:profile.apiKey,ratio,duration,resolution:resolution==='固定'?undefined:resolution,size:IMAGE_FORMATS.find(x=>x.value===ratio)?.size, references:refs});
    }catch(e){setError(e.message);}finally{lock.current=false;setBusy(false);}
  };
  return <div className="generation-composer"><div className="generation-editor"><div className="generation-prompt-area"><label>画面与镜头描述 <small>{value.prompt?.length || 0} 字</small></label><textarea aria-label="提示词" placeholder="描述画面、动作、镜头与声音；点击素材编号可插入引用…" value={value.prompt || ''} onChange={e=>change({prompt:e.target.value})} disabled={disabled}/><div className="generation-references">{numberedReferences(refs).map((r,i)=><figure key={r.id || i}>{r.kind==='image'?<img src={mediaSource(r)} alt={r.name}/>:r.kind==='video'?<video src={mediaSource(r)} controls preload="metadata"/>:<audio src={mediaSource(r)} controls preload="metadata"/>}<button className="reference-remove" aria-label={`移除${r.name}`} disabled={disabled} onClick={()=>change({references:refs.filter((_,n)=>n!==i)})}><X size={12}/></button><button className="reference-alias" disabled={disabled} onClick={()=>change({prompt:`${value.prompt || ''} ${r.alias}`})}>{r.alias}</button><figcaption>{r.name}</figcaption>{model.protocol==='metadata-content'&&r.kind==='image'&&<select aria-label="图片用途" value={r.role||'reference_image'} onChange={e=>change({references:refs.map((item,n)=>n===i?{...item,role:e.target.value}:item)})}><option value="reference_image">参考图</option><option value="first_frame">首帧</option><option value="last_frame">尾帧</option></select>}</figure>)}<button className="collab-add-ref" aria-label="添加参考素材" disabled={disabled||importing} onClick={()=>onPick?onPick():importFile('image')}><Plus/></button></div>{children}</div><div className="generation-controls"><label>生成接口<select aria-label="生成接口" value={profile?.id||''} onChange={e=>change({profileId:e.target.value,model:''})}>{!profiles.length&&<option>请先配置 API</option>}{profiles.map(p=><option key={p.id} value={p.id}>{p.name}</option>)}</select></label><label>模型<select aria-label="模型" value={model.id} onChange={e=>change({model:e.target.value})}>{available.map(m=><option key={m.id} value={m.id}>{m.name}</option>)}</select></label><div className="generation-param-grid"><label>画面比例<select aria-label="比例" value={ratio} onChange={e=>change({ratio:e.target.value})}>{caps.ratios.map(r=><option key={r}>{r}</option>)}</select></label>{kind==='video'&&<label>视频时长<select aria-label="时长" value={duration} onChange={e=>change({duration:+e.target.value})}>{durations.map(d=><option key={d} value={d}>{d} 秒</option>)}</select></label>}{kind==='video'&&<label>分辨率<select aria-label="分辨率" value={resolution} onChange={e=>change({resolution:e.target.value})}>{resolutions.map(r=><option key={r}>{r}</option>)}</select></label>}</div>{feituo&&<small className="generation-limits">图片 {caps.maxImages ?? '不限'} 张 · 视频 {caps.maxVideos} 个 · 音频 {caps.maxAudios} 个<br/>{model.price}<br/>以平台实际结算为准</small>}<div className="generation-upload-buttons">{['image',...(kind==='video'?['audio','video']:[])].map(k=><button key={k} disabled={disabled || importing || (feituo && caps[k==='image'?'maxImages':k==='audio'?'maxAudios':'maxVideos']===0)} onClick={()=>onPick?onPick(k):importFile(k)}><Upload size={14}/>{k==='image'?'参考图片':k==='audio'?'参考音频':'参考视频'}</button>)}</div><button className="primary" disabled={disabled||busy||!value.prompt?.trim()||!profile} onClick={run}><Sparkles size={16}/>{busy?'正在提交…':kind==='video'?'生成视频':'生成图片'}</button>{error&&<p className="collab-error" role="alert">{error}</p>}</div></div></div>;
}
